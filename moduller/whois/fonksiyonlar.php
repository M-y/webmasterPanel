 ...
 * 
 * @p domain: sorgulanacak domain
 * 
 * @return whois bilgisi varsa whois yoksa boş
 */
class Whois{
private $whois_sunuculari = array(
...
{
$domain = trim($domain);
if(substr(strtolower($domain), 0, 7) == "http://") $domain = substr($domain, 7); //http varsa kaldır
if(substr(strtolower($domain), 0, 4) == "www.") $domain = substr($domain, 4);//www varsa kaldır
if(preg_match("/^(([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\.){3}([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])$/",$domain))
return $this->whoisSorgu("whois.lacnic.net",$domain);
elseif(preg_match("/^([-a-z0-9]{2,100})\.([a-z\.]{2,8})$/i",$domain))
{
$tld = strtolower(array_pop(explode(".", $domain)));
$sunucu = $this->whois_sunuculari[$tld][0];
if(!$sunucu):
return $domain.": Domain için uygun whois sunucusu bulunamadı!";
endif;
$res=$this->whoisSorgu($sunucu,$domain);
while(preg_match("/Whois Server: (.*)/", $res, $eslesme)):
$res=$this->whoisSorgu($eslesme[1],$domain);
endwhile;
return $res;
}
else
...
private function whoisSorgu($sunucu,$domain)
{
$fp = @fsockopen($sunucu, 43, $errno, $errstr, 20) or die("Socket Hatası " . $errno . " - " . $errstr);
fputs($fp, $domain . "\r\n");
$out = "";
while(!feof($fp)):
$out .= fgets($fp);
